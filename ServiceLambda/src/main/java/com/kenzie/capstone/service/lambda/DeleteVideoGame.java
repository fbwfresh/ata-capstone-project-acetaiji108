...
public class DeleteVideoGame implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
       ...
       // JsonStringToVideoGameConverter jsonStringToVideoGameConverter = new JsonStringToVideoGameConverter();
        ...
        try {
           // VideoGameRequest videoGameRequest = jsonStringToVideoGameConverter.convert(input.getBody());
            boolean deleteVideoGame = videoGameService.deleteVideoGame(input.getBody());

            return response
                    .withStatusCode(200)
                    .withBody(gson.toJson(deleteVideoGame));
